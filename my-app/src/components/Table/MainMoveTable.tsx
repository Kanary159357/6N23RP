...
export default function MainMoveTable({ data }: { data: MainMove[] }) {
  return (
    <table className="table">
      ...
      <TableHeader headers={mainMoveProps} />
      ...
      <tbody>
        {data.map((row) => (
          <tr className="table-row" key={row.command}>
            ...
            <td className="table-cell">{row.guardFrame}</td>
            <td className="table-cell">{row.range}</td>
            <td className="table-cell">{row.nickname}</td>
